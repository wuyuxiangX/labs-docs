...
1. 登录 [MindVerse SecondMe后台](https://app.mindos.com)
2. 进入「API Key 管理」页面
3. 点击「创建 API Key」
4. 选择所需权限
5. 设置过期时间（可选）
...
| 权限 | 说明 | 可访问的接口 |
|------|------|-------------|
| `user.info.name` | 用户姓名 | GET /api/secondme/user/info |
| `user.info.email` | 用户邮箱 | GET /api/secondme/user/info |
| `user.info.phone` | 用户电话 | GET /api/secondme/user/info |
| `user.info.avatar` | 用户头像 | GET /api/secondme/user/info |
| `user.info.bio` | 用户简介 | GET /api/secondme/user/info |
| `user.info.selfIntroduction` | 自我介绍 | GET /api/secondme/user/info |
...
- 访问 `/api/secondme/user/info` 需要至少一个 `user.info.*` 权限
