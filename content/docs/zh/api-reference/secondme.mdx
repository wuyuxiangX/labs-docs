...
| voice_id | string | 是 | 语音 ID（预置或克隆音色） |
| model | string | 否 | TTS 模型：`speech-2.6-hd`（默认）、`speech-02-hd` |
| emotion | string | 否 | 情绪：`happy`/`sad`/`angry`/`fearful`/`disgusted`/`surprised`/`calm`/`fluent`（默认） |
| speed | number | 否 | 语速 0.5-2.0（默认 1.0） |
| volume | number | 否 | 音量 0-10（默认 1.0） |
| pitch | integer | 否 | 音调 -12 到 12（默认 0） |
...
| X-Audio-Duration-Ms | 音频时长（毫秒） |
| X-Audio-Sample-Rate | 采样率 |
