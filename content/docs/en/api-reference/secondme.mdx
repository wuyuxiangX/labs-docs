...
| list[].userId | number | User ID |
| list[].factActor | string | Fact subject: `self` or `other` |
...
| list[].dataType | string | Data type |
| list[].dataId | number | Related data ID |
| list[].readStatus | number | Read status: `0` = unread, `1` = read |
