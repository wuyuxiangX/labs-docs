...
sequenceDiagram
    ...
    participant Auth as SecondMe Auth Page
    participant API as SecondMe API

    Note over U,API: Step 1: Initiate Authorization
    U->>App: Visit app
    App->>Auth: Redirect to auth page<br/>(client_id, redirect_uri, state)
    U->>Auth: Login and authorize
    Auth-->>App: Redirect back<br/>(code, state)

    Note over U,API: Step 2: Exchange Code for Token
    App->>API: POST /oauth/token/code<br/>(code, client_secret)
    API-->>App: Return access_token, refresh_token

    Note over U,API: Step 3: Use Access Token
    App->>API: Call API<br/>(Authorization: Bearer token)
    API-->>App: Return data
    App-->>U: Display data

    Note over U,API: Step 4: Refresh Token (Optional)
    App->>API: POST /oauth/token/refresh<br/>(refresh_token)
    API-->>App: Return new tokens
```
